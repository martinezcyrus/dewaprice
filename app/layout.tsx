        ...
        <GuestContext.Provider value={{ isGuest, isAdmin, showPermissionModal }}>
          ...
          <nav style={{ background: 'linear-gradient(135deg, #0d2137, #1a3d5c)', color: 'white', padding: '0 20px', display: 'flex', alignItems: 'center', height: '56px', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 12px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'flex', gap: '4px', flex: 1 }}>
              {navLinks.map(link => (
                <a key={link.href} href={link.href} style={{ color: pathname === link.href ? 'white' : 'rgba(255,255,255,0.6)', textDecoration: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: pathname === link.href ? '600' : '400', background: pathname === link.href ? 'rgba(255,255,255,0.15)' : 'transparent' }}>
                  {link.label}
                </a>
              ))}
            </div>
