  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#0d2137', margin: '0 0 10px 0' }}>
          Access Restricted
        </h2>
        ...
        <p style={{ fontSize: '13px', color: '#888', lineHeight: '1.7', margin: '0 0 24px 0' }}>
          You're in <strong>Demo Mode</strong> — all features are visible
        ...
        <div style={{
          ...
          <div style={{ fontSize: '10px', color: '#999', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '1px' }}>
            Request Access From
          </div>
...
function DemoBanner() {
  ...
  if (minimized) return (
    <div onClick={() => setMinimized(false)} style={{
      ...
      zIndex: 1000, boxShadow: '0 4px 16px rgba(230,81,0,0.4)',
      display: 'flex', alignItems: 'center', gap: '6px',
      animation: 'pulse 2s infinite'
    }}>
      <style>{`@keyframes pulse { 0%,100%{box-shadow:0 4px 16px rgba(230,81,0,0.4)} 50%{box-shadow:0 4px 24px rgba(230,81,0,0.7)} }`}</style>
      👁️ Demo Mode
    </div>
  )
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div>
          <strong>Demo Mode</strong> — You're viewing DewaPrice as a read-only guest.
          <span style={{ opacity: 0.8, marginLeft: '6px' }}>All features visible · Editing disabled.</span>
        </div>
      </div>
    ...
    <html lang="en">
      <body style={{ margin: 0, fontFamily: 'Arial, sans-serif' }}>
        <GuestContext.Provider value={{ isGuest, isAdmin, showPermissionModal }}>
          ...
          {permissionModal && (
            <PermissionModal action={permissionModal} onClose={() => setPermissionModal(null)}/>
          )}
          ...
          <nav style={{
            ...
            <div style={{ fontWeight: '700', fontSize: '16px', marginRight: '32px', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              💧 DewaPrice
              {isGuest && (
                <span style={{ fontSize: '10px', background: '#E65100', color: 'white', padding: '2px 8px', borderRadius: '99px', fontWeight: '700' }}>
                  DEMO
                </span>
              )}
              {isAdmin && (
                <span style={{ fontSize: '10px', background: '#2E7D32', color: 'white', padding: '2px 8px', borderRadius: '99px', fontWeight: '700' }}>
                  ADMIN
                </span>
              )}
            </div>
            ...
            <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {sidebarOpen && (
            <div style={{ position: 'fixed', inset: 0, zIndex: 200 }}>
              ...
              <div style={{
                ...
                {navLinks.map(link => (
                  <a key={link.href} href={link.href} onClick={() => setSidebarOpen(false)} style={{
                ...
                {!isGuest ? (
                  ...
                  <a href="mailto:cyrusjaysonm@gmail.com?subject=DewaPrice Access Request&body=Hi Cyrus,%0A%0AI viewed your DewaPrice demo and would like to request full access.%0A%0ARegards"
                    style={{
                      ...
                      textDecoration: 'none', display: 'block', fontWeight: '700',
