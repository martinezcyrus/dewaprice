...
function PermissionModal({ action, onClose }: { action: string, onClose: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }} onClick={onClose}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '36px', maxWidth: '420px', width: '100%', boxShadow: '0 32px 80px rgba(0,0,0,0.3)', textAlign: 'center', animation: 'popIn 0.2s ease' }} onClick={e => e.stopPropagation()}>
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #E65100, #BF360C)', color: 'white', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' as const, gap: '8px', fontSize: '13px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div><strong>Demo Mode</strong> — Read-only guest view. <span style={{ opacity: 0.8 }}>All features visible · Editing disabled.</span></div>
      </div>
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Safety net — never stuck loading more than 4 seconds
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_OUT') {
        setUser(null)
        setProfile(null)
        // Small delay to let Supabase finish cleanup
  ...
  // Only show full-screen loader on very first load before session is known
  ...
  // No user — show page without nav (login/demo will render their own UI)
