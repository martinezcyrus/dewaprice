        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '0' }}>
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => tab.active && setActiveTab(tab.id)}
              style={{ padding: '10px 20px', borderRadius: '8px 8px 0 0', border: 'none', fontSize: '13px', fontWeight: '600', cursor: tab.active ? 'pointer' : 'not-allowed', background: activeTab === tab.id ? 'white' : '#e0e0e0', color: activeTab === tab.id ? '#0d2137' : '#999', opacity: tab.active ? 1 : 0.6, boxShadow: activeTab === tab.id ? '0 -2px 8px rgba(0,0,0,0.06)' : 'none' }}>
