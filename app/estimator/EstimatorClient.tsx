...
export default function EstimatorClient({ userId, initialPrices, initialEstimates }: Props) {
  ...
  const [dbPrices, setDbPrices] = useState<any[]>(initialPrices)
  const [savedEstimates, setSavedEstimates] = useState<any[]>(initialEstimates)
  ...
  const calculate=()=>{
    ...
    const addItem=(sectionId:string,description:string,unit:string,qty:number,dur:number,unitCost:number,markupPct:number)=>{
      ...
      equipment.push({id:`${sectionId}-${equipment.length}`,section:SECTION_LABELS[sectionId]||sectionId,sectionId,description,unit,quantity:qty,duration:dur,unitCost,totalCost,markupPct,sellingPrice,margin})
    }
  ...
  const handleDownloadBOQ=async()=>{
    ...
    try {
      ...
      for (const sid of sectionOrder) {
        ...
        boqRows.push(['','','','',`Subtotal — ${SECTION_LABELS[sid]?.replace(/^\w+\.\s*/,'')||sid}`,{ v: secTotal, t: 'n', z: `"${sym}"#,##0.00` }])
        boqRows.push([])
        grandTotal += secTotal
        itemIdx = 1
      }
      ...
      for (const sid of sectionOrder) {
        ...
        costRows.push(['','','','',`Subtotal`,{ v:secCost, t:'n', z:`"${sym}"#,##0.00` },'',{ v:secSell, t:'n', z:`"${sym}"#,##0.00` },{ v:secSell>0?(secSell-secCost)/secSell:0, t:'n', z:'0.0%' }])
        costRows.push([])
      }
      costRows.push([])
            ...
            <div>
              ...
              <div style={sectionStyle}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(3, 1fr)',gap:'14px'}}>
                  ...
                  <div><label style={labelStyle}>Risk Level</label><select {...inp('riskLevel')} style={inputStyle}><option>Low</option><option>Medium</option><option>High</option></select><div style={{fontSize:'11px',color:'#999',marginTop:'4px'}}>SF: {inputs.riskLevel==='High'?'2.0':inputs.riskLevel==='Medium'?'1.5':'1.25'}×</div></div>
                  <div style={{display:'flex',alignItems:'flex-end'}}><div style={{fontSize:'12px',color:'#666',padding:'10px',background:'#f8f9fa',borderRadius:'8px',width:'100%'}}>Project days: <strong>{Math.round(parseFloat(inputs.projectDuration||'0')*7)}</strong><br/>Months: <strong>{Math.ceil(parseFloat(inputs.projectDuration||'0')/4)}</strong></div></div>
                </div>
              </div>
              <div style={sectionStyle}>
              ...
              <div style={sectionStyle}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:'10px'}}>
                  {[{key:'mobilization',label:'A. Mobilization',color:SECTION_COLORS['A']},{key:'drilling',label:'2. Drilling & Install',color:SECTION_COLORS['2']},{key:'installation',label:'3. Installation',color:SECTION_COLORS['3']},{key:'rental',label:'4. Rental',color:SECTION_COLORS['4']},{key:'diesel',label:'5. Diesel Fuel',color:SECTION_COLORS['5']},{key:'om',label:'6. O&M',color:SECTION_COLORS['6']},{key:'demobilization',label:'7. Demobilization',color:SECTION_COLORS['7']}].map(({key,label,color})=>(
                    <div key={key} style={{background:`${color}11`,border:`1.5px solid ${color}33`,borderRadius:'10px',padding:'12px'}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                        <input type="number" value={markups[key as keyof CategoryMarkups]} onChange={e=>setMarkups({...markups,[key]:parseFloat(e.target.value)||0})} style={{...inputStyle,padding:'6px 10px',fontSize:'16px',fontWeight:'700',color,textAlign:'right' as const,background:'white'}}/>
                        <span style={{fontSize:'16px',fontWeight:'700',color}}>%</span>
                      </div>
                      <div style={{fontSize:'10px',color:'#999',marginTop:'4px'}}>Margin: {(markups[key as keyof CategoryMarkups]/(100+markups[key as keyof CategoryMarkups])*100).toFixed(1)}%</div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={sectionStyle}>
                ...
                <label style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'13px',cursor:'pointer',marginBottom:'12px'}}>
                  <input type="checkbox" checked={overrideMethod} onChange={e=>setOverrideMethod(e.target.checked)}/>Override — select method manually
                </label>
                {overrideMethod&&(
              ...
              <div style={{position:'sticky' as const,top:'76px'}}>
                <div style={{background:'white',borderRadius:'12px',padding:'16px',boxShadow:'0 2px 8px rgba(0,0,0,0.06)',border:'1px solid #e8edf2'}}>
                  ...
                  <div style={{marginTop:'12px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                    {[{label:'Perimeter',value:`${(2*((parseFloat(inputs.excavationLength)||0)+(parseFloat(inputs.excavationWidth)||0))).toFixed(0)}m`},{label:'Area',value:`${((parseFloat(inputs.excavationLength)||0)*(parseFloat(inputs.excavationWidth)||0)).toFixed(0)}m²`},{label:'Drawdown',value:`~${Math.max(0,(parseFloat(inputs.excavationDepth)||0)-(parseFloat(inputs.groundwaterDepth)||0)+parseFloat(inputs.requiredDrawdown||'0.5')).toFixed(1)}m`},{label:'k',value:`${(inputs.useCustomK&&inputs.permeabilityK?parseFloat(inputs.permeabilityK):SOIL_K[inputs.soilType]||1e-4).toExponential(0)} m/s`}].map(stat=>(
        ...
        {step===2&&(
          <div>
            <div style={sectionStyle}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',alignItems:'start'}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                  {[{label:'Project',value:inputs.projectName||'Unnamed'},{label:'Location',value:inputs.location||'-'},{label:'Excavation',value:`${inputs.excavationLength}×${inputs.excavationWidth}×${inputs.excavationDepth}m`},{label:'GWT Depth',value:`${inputs.groundwaterDepth}m`},{label:'Soil Type',value:inputs.soilType},{label:'k',value:`${(inputs.useCustomK?parseFloat(inputs.permeabilityK):SOIL_K[inputs.soilType]||1e-4).toExponential(0)} m/s`},{label:'Duration',value:`${inputs.projectDuration} weeks`},{label:'Risk Level',value:inputs.riskLevel},{label:'Method',value:overrideMethod&&selectedMethod?`${selectedMethod} (manual)`:'Auto-select'},{label:'Diesel',value:`${inputs.dieselLitersPerHrPerPump} L/hr/pump × ${inputs.operatingHrsPerDay} hrs/day`}].map(item=>(
                ...
                <div>
                  <div style={{background:'#f8f9fa',borderRadius:'10px',padding:'12px',marginBottom:'12px'}}><CrossSection inputs={inputs}/></div>
                  <div style={{padding:'12px 14px',background:dbPrices.length>0?'#E8F5E9':'#FFF3E0',borderRadius:'8px',fontSize:'12px'}}>
                    {dbPrices.length>0?<span style={{color:'#2E7D32'}}>✅ {dbPrices.length} prices loaded · {dbPrices.filter(p=>Array.isArray(p.method_tags)&&p.method_tags.length>0).length} tagged</span>:<span style={{color:'#E65100'}}>⚠️ No prices loaded — will use default fallback prices</span>}
                  </div>
                </div>
              </div>
            </div>
            <div style={{display:'flex',gap:'12px'}}>
              ...
              <button onClick={calculate} disabled={calculating} style={{flex:1,padding:'14px',background:calculating?'#90CAF9':'linear-gradient(135deg, #1565C0, #0288D1)',color:'white',border:'none',borderRadius:'10px',fontSize:'15px',fontWeight:'600',cursor:'pointer'}}>{calculating?'⏳ Calculating...':'⚡ Run Calculations →'}</button>
            </div>
          </div>
        )}
        ...
        {step===3&&results&&(
          <div>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap' as const,gap:'12px'}}>
                ...
                <div style={{display:'flex',gap:'8px',flexWrap:'wrap' as const,alignItems:'center'}}>
                  <div style={{display:'flex',gap:'4px',background:'#f0f4f8',borderRadius:'8px',padding:'4px'}}>
                    {CURRENCIES.map(c=>(<button key={c.code} onClick={()=>setCurrency(c)} style={{padding:'6px 10px',background:currency.code===c.code?'white':'transparent',color:currency.code===c.code?'#0d2137':'#666',border:'none',borderRadius:'6px',fontSize:'12px',fontWeight:currency.code===c.code?'700':'400',cursor:'pointer',boxShadow:currency.code===c.code?'0 1px 4px rgba(0,0,0,0.1)':'none',transition:'all 0.15s'}}>{c.flag} {c.code}</button>))}
                  </div>
                  ...
                  <button onClick={handleDownloadBOQ} disabled={generatingBoq} style={{padding:'8px 16px',background:'#2E7D32',color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer'}}>{generatingBoq?'⏳ Generating...':'📥 Download BOQ'}</button>
                </div>
              </div>
            </div>
            <div style={sectionStyle}>
            ...
            <div style={sectionStyle}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px',flexWrap:'wrap' as const,gap:'10px'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                  ...
                  <button onClick={handleDownloadBOQ} disabled={generatingBoq} style={{padding:'8px 16px',background:'#2E7D32',color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer'}}>{generatingBoq?'⏳':'📥 Download BOQ'}</button>
                </div>
              </div>
                  ...
                  <div key={sectionId} style={{marginBottom:'20px'}}>
                    ...
                    <div style={{overflowX:'auto' as const}}>
                      <table style={{width:'100%',borderCollapse:'collapse',fontSize:'12px'}}>
                        <thead><tr style={{background:'#f8f9fa',borderBottom:'1.5px solid #e0e0e0'}}>{['Description','Unit','Qty','Unit Cost','Total Cost','Markup%','Selling Price','Margin'].map(h=>(<th key={h} style={{padding:'8px 10px',textAlign:'left' as const,fontWeight:'700',color:'#555',fontSize:'10px',textTransform:'uppercase' as const,whiteSpace:'nowrap' as const}}>{h}</th>))}</tr></thead>
                        <tbody>
                          {items.map((item,i)=>(
                        ...
                        <tfoot><tr style={{background:`${secColor}11`,borderTop:`2px solid ${secColor}44`}}><td colSpan={4} style={{padding:'8px 10px',fontWeight:'700',color:secColor,fontSize:'11px',textTransform:'uppercase' as const}}>Subtotal</td><td style={{padding:'8px 10px',fontWeight:'700',color:secColor}}>{fmt(secCost)}</td><td/><td style={{padding:'8px 10px',fontWeight:'700',color:secColor}}>{fmt(secSell)}</td><td style={{padding:'8px 10px',fontWeight:'700',color:secColor}}>{(secMargin*100).toFixed(1)}%</td></tr></tfoot>
                      </table>
                    </div>
                  </div>
                )
              ...
              <div style={{marginTop:'16px',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:'12px'}}>
                <div style={{background:'#f8f9fa',borderRadius:'10px',padding:'16px 20px'}}><div style={{fontSize:'11px',color:'#999',fontWeight:'700',textTransform:'uppercase' as const,marginBottom:'6px'}}>Total Cost Price</div><div style={{fontSize:'24px',fontWeight:'bold',color:'#0d2137'}}>{fmt(results.totalCostPrice)}</div><div style={{fontSize:'12px',color:'#999',marginTop:'2px'}}>Before markup</div></div>
                <div style={{background:'#f8f9fa',borderRadius:'10px',padding:'16px 20px'}}><div style={{fontSize:'11px',color:'#999',fontWeight:'700',textTransform:'uppercase' as const,marginBottom:'6px'}}>Gross Profit</div><div style={{fontSize:'24px',fontWeight:'bold',color:'#E65100'}}>{fmt(results.totalSellingPrice-results.totalCostPrice)}</div><div style={{fontSize:'12px',color:'#999',marginTop:'2px'}}>Margin: {(results.grossMargin*100).toFixed(1)}%</div></div>
                <div style={{background:`${methodColor}11`,border:`2px solid ${methodColor}`,borderRadius:'10px',padding:'16px 20px'}}><div style={{fontSize:'11px',color:methodColor,fontWeight:'700',textTransform:'uppercase' as const,marginBottom:'6px'}}>Total Selling Price ({currency.code})</div><div style={{fontSize:'28px',fontWeight:'bold',color:methodColor}}>{fmt(results.totalSellingPrice)}</div><div style={{fontSize:'12px',color:methodColor,opacity:0.7,marginTop:'2px'}}>Client price incl. markup</div></div>
              </div>
            </div>
            <div style={sectionStyle}>
            ...
            <div style={{display:'flex',gap:'10px',flexWrap:'wrap' as const}}>
              ...
              <button onClick={handleDownloadBOQ} disabled={generatingBoq} style={{padding:'12px 20px',background:'#2E7D32',color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer'}}>{generatingBoq?'⏳ Generating...':'📥 Download BOQ (.xlsx)'}</button>
